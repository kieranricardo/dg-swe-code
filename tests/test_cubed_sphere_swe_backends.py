...
from dg_swe.numba_kernels import _solve_numba_kernel


pytestmark = pytest.mark.skipif(
    _solve_numba_kernel is None,
    reason="numba is required to compare cubed-sphere SWE NumPy backends",
)
...
def _make_solvers(
    ...
    return None, DGCubedSphereSWE(**common_kwargs, flux_type=flux_type)


def _set_random_state(numpy_solver, seed):
    rng = np.random.default_rng(seed)

    for name in FACE_NAMES:
        shape = numpy_solver.faces[name].J.shape
...
def test_numpy_and_numba_residuals_match(poly_order, grid, ah, tangent_diss):
    _, numpy_solver = _make_solvers(poly_order, grid, tangent_diss, ah=ah, a=ah)
    _, numpy_state = _set_random_state(
        numpy_solver, seed=1100 + 100 * poly_order + grid
    )
...
def test_numpy_and_numba_residuals_match_with_old_tangent_diss(poly_order, grid, ah):
    ...
    _, numpy_state = _set_random_state(
        numpy_solver, seed=3100 + 100 * poly_order + grid
    )
...
def test_numpy_and_numba_residuals_match_with_lmars(poly_order, grid, ah):
    ...
    _, numpy_state = _set_random_state(
        numpy_solver, seed=4100 + 100 * poly_order + grid
    )
...
def test_numpy_and_numba_residuals_match_with_barth_diss(poly_order, grid, flux_type):
    ...
    _, numpy_state = _set_random_state(
        numpy_solver, seed=5100 + 100 * poly_order + grid
    )
...
def test_benchmark_numpy_and_numba_residual_passes():
    _, numpy_solver = _make_solvers(poly_order=3, grid=8, tangent_diss=True)
    _, numpy_state = _set_random_state(numpy_solver, seed=20260731)
    ...
    print(f"  numpy solve: {numpy_time:.6f} s/pass")
    print(f"  numba solve: {numba_time:.6f} s/pass")
